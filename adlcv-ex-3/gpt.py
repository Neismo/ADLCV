...
class MaskedAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_logits = queries @ keys.mT/self.scale
        ...
        mask = torch.triu(torch.ones(attention_logits.size(-1), attention_logits.size(-1)), diagonal=1).bool().to(attention_logits.device)
